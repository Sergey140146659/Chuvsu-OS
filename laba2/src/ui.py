...
class CLI:
    ...
    def _display_stats(self):
        ...
        print("--- Эмулятор Операционной Системы (Лаб. 2) ---")
        print(f"Скорость: {stats['speed_hz']} такт/сек | Память: {stats['memory_usage']} | Процессы: {stats['process_count']}")

        print("\n--- Таблица Слов Состояний Процессов (PSW) ---")
        print(f"{'PID':<5} | {'Состояние':<12} | {'PC':<7} | {'Размер':<7}")
        print("-" * 45)
        ...
        if stats['all_processes']:
            ...
            for proc in sorted_processes:
                ...
                print(f"{pid_str:<5} | {state_str:<12} | {pc_str:<7} | {size_str:<7}")
        else:
