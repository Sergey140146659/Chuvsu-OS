...
class CLI:
    ...
    def _display_stats(self, stats):
        ...
        self._display_performance_analysis(stats)
    ...
    def _display_process_table(self, stats):
            ...
            header = f"{'PID':<5} | {'Состояние':<12} | {'PC':<7} | {'Тики':<5} | {'I/O':<5} | {'T_wait':<8} | {'T_mono':<8} | {'Размер':<7}"
            print(header)
            print("-" * len(header))
            ...
            if stats['all_processes']:
                ...
                for proc in sorted_processes:
                    print(f"{proc.pid:<5} | {proc.state.value:<12} | {proc.program_counter:<7} | {proc.ticks_worked_in_quantum:<5} | {proc.io_time_remaining:<5} | {proc.wait_time:<8} | {proc.mono_execution_time:<8} | {proc.size:<7}")
            else:
    ...
    def _display_performance_analysis(self, stats):
        """Вывод анализа производительности и отчета по последнему процессу."""
        print("\n[Анализ Производительности]")
        print(f"  Всего завершено: {stats['completed_count']}")
        print(f"  Среднее T_wait: {stats['avg_wait_time']:.2f} | Среднее T_multi: {stats['avg_turnaround_time']:.2f} | Среднее T_mono: {stats['avg_mono_time']:.2f}")
        print(f"  > Производительность (T_mono / T_multi): {stats['performance_ratio']:.2f}%")

        report = stats.get('last_process_report')
        if report:
            print("\n[Отчет по последнему завершенному процессу]")
            print(f"  PID: {report['pid']} | T_multi: {report['t_multi']} | T_mono: {report['t_mono']} | T_wait: {report['wait_time']} | Накладные расходы: {report['overhead']} тактов")
