...
class ProcessManager:
    ...
    def create_and_register_process(self, size: int, program_length: int, creation_tick: int) -> Optional[Process]:
        if self.is_table_full():
            return None

        new_process = Process(size=size, program_length=program_length, creation_tick=creation_tick)
