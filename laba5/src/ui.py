...
class CLI:
    ...
    def _display_stats(self):
        ...
        print("--- Эмулятор Операционной Системы (Лаб. 5) ---")
        print(f"CPU: {stats['cpu_state']} | Активный PID: {stats['active_pid']} | Последняя команда: {stats['last_command']}")
        print(f"Процессы: {stats['process_count']} (Готовых: {stats['ready_count']}, Заблокировано: {stats['blocked_count']})")
        print(f"Память: {stats['memory_usage']} | Скорость: {stats['speed_hz']} такт/сек")
        print(f"Очередь прерываний: {stats['interrupt_queue']}")
        ...
        if stats['next_task']:
            ...
            print("Следующее задание на загрузку: не сгенерировано (память или таблица полны).")
